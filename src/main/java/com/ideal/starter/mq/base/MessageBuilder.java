...
import com.ideal.starter.mq.annotation.MQKey;
import com.ideal.starter.mq.enums.DelayTimeLevel;
import lombok.Data;
...
public class MessageBuilder {
    ...
    public Message build() {
        ...
        if (message instanceof String) {
            body = ((String) message).getBytes(Charset.forName(charset));
        } else {
            try {
                String jsonObj = gson.toJson(message);
                body = jsonObj.getBytes(Charset.forName(charset));
            } catch (Exception e) {
